...
return(<React.Fragment>
    <section className={`px-4 ${show ? 'h-screen' : 'h-16'}  top-0 main fixed z-30 left-0 right-0 transition-all delay-400 duration-300 pt-2 lg:h-20 lg:px-10 w-full flex flex-col lg:flex-row lg:justify-between lg:items-center`}>
        <div className='flex flex-row justify-between w-full items-center'>
            <Link to='/'>
            <h1 className=' text-white font-bold text-2xl' onClick={()=> showHandler('home')}>
            mayhoral.dev
            </h1>
            </Link>
        {!show ? <FiMenu onClick={()=> showHandler('menu')} className='text-white cursor-pointer font-bold text-2xl lg:hidden'/> : <FaXmark onClick={()=> showHandler('menu')} className='text-white cursor-pointer font-bold text-2xl lg:hidden'/> }
        </div>
